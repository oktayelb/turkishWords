...
class InteractiveTrainer:
    ...
    def __init__(self, use_triplet_loss: bool = False, use_lstm: bool = False):
        ...
        self.training_count = FileManager()._load_training_count()        
        self.auto_stats = {
            'words_processed': 0,
            'words_deleted': 0,
            'words_skipped': 0
    ...
    def decompose(word):
        ...
        for i in range(1, len(word) + 1):
            root = word[:i]
            if not wrd.exists(root):                                    # Bu iki satiri yoruma alirsan sozlukte olmayan yepyeni koklerle de is yapabilirsin
    ...
    def train_on_word(self, word: str) -> Optional[bool]:
        ...
        FileManager()._log_decompositions(word, correct_indices, decompositions)
        
        # Delete word if its root exists in dictionary
        if FileManager()._delete_word_if_root_exists(word, correct_indices, decompositions):
    ...
    def batch_train_from_file(self, filepath: Optional[str] = None):
        ...
        # Load entries
        entries =  FileManager().get_valid_decomps()
...
class FileManager:
    ...
    def __init__(self):
        ...
        with open(self.config.words_path, "r", encoding="utf-8") as f:
            self.words = list(line.strip() for line in f if line.strip())
    ...
    def _delete_word_if_root_exists(self, word: str, correct_indices: List[int], 
        ...
        for idx in correct_indices:
            ...
            if wrd.exists(root) > 0:
                ...
                if to_be_deleted and self.delete(to_be_deleted):
    ...
    def delete(self,word):
        ...
        try:
            ...
            with open(self.config.words_path, "w",encoding="utf-8") as file:
                for w in self.words:
                    file.write(w + "\n")
    ...
    def exists(self,word) -> bool:
        ...
        if wrd.infinitive(word)  in self.words:
            return 2
        ...
        if word.endswith("l"):
            ...
            if wrd.infinitive(soft_l) in self.words:
                return 2 
